...
Boon::OpenGLFramebuffer::OpenGLFramebuffer(const FramebufferDescriptor& desc)
	: m_Desc(desc)
{
	for (auto spec : m_Desc.Attachments.Attachments)
	{
		if (!Utils::IsDepthFormat(spec.TextureFormat))
			m_ColorAttachmentDescriptors.emplace_back(spec);
		else
			m_DepthAttachmentDescriptor = spec;
	}
...
void Boon::OpenGLFramebuffer::Invalidate()
{
	if (m_ID)
	{
		glDeleteFramebuffers(1, &m_ID);
	...
	if (m_DepthAttachmentDescriptor.TextureFormat != FramebufferTextureFormat::None)
	{
		Utils::CreateTextures(multisample, &m_DepthAttachment, 1);
		Utils::BindTexture(multisample, m_DepthAttachment);
		switch (m_DepthAttachmentDescriptor.TextureFormat)
		{
...
void Boon::OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
{
	auto& spec = m_ColorAttachmentDescriptors[attachmentIndex];
